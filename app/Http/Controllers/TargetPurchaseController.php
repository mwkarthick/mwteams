...
use App\Models\TargetPurchaseModel;
use App\Models\TargetPurchaseLineModel;


class TargetPurchaseController extends Controller {

    public $modelName = 'App\Models\TargetPurchaseModel';
    public $modelLineName = 'App\Models\TargetPurchaseLineModel';
    public $modelTeamName = 'App\Models\TeamModel';
    public $baseRedirect = 'targetpurchase.index';
    public $baseName = 'targetpurchase';
    public $basePath = 'targetpurchase/';
    public $detailName = 'TargetPurchaseController@getIndex';
    ...
    public function uploadcsv(Request $request) {
        ...
        {   $k=0;
            ...
            if($modelData)
            {
                ...
                $modelPurchase = $model->where('team_code',$inputs['team_code'][$i])
                                ->where('startdate','=',$inputs['startdate'])
                                ->where('enddate','=',$inputs['enddate'])->first();
                if($modelPurchase)
                {
                        $inp['target_purchase_id']= $modelPurchase->id;
                ...
                {
                    $modelPurchase2 = $model->where('team_code',$inputs['team_code'][$i])
                                ->where('enddate','>',$inputs['startdate'])->get();
                    if(is_array($modelPurchase2))
                    {
                        foreach($modelPurchase2 as $modelPur)
                        {
                    ...
                    else
                    {
                        ...
                        {
                            $inp['target_purchase_id']= $model->id;
                            $inp['product_code']= $inputs['product_code'][$i];
                            $inp['product_name']= $inputs['product_name'][$i];
                            $inp['target_value']= $inputs['target_value'][$i];
                            $inp['created_by_id']= session()->get('user_id');
